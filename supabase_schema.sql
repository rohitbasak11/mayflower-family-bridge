...
CREATE TABLE IF NOT EXISTS public.profiles (
    id uuid PRIMARY KEY DEFAULT uuid_generate_v4(), -- ID can be generated or from auth.users
    email text UNIQUE NOT NULL, -- Email must be unique for claim logic
    full_name text,
    avatar_url text,
...
RETURNS trigger AS $$
BEGIN
  -- 1. If Staff already created a profile for this email, "claim" it
...
-- VITALS & MEDICATIONS
DROP POLICY IF EXISTS "health_select" ON public.vitals;
CREATE POLICY "health_select" ON public.vitals FOR SELECT USING (auth.uid() = resident_id OR is_staff());
DROP POLICY IF EXISTS "meds_select" ON public.medications;
CREATE POLICY "meds_select" ON public.medications FOR SELECT USING (auth.uid() = resident_id OR is_staff());
...
-- Enable storage RLS indirectly (Supabase default) but ensure policies exist
-- NOTE: You must create 'chat-photos' and 'resident-gallery' buckets in the UI first.

-- CHAT PHOTOS
DROP POLICY IF EXISTS "View Chat Photos" ON storage.objects;
CREATE POLICY "View Chat Photos" ON storage.objects 
  FOR SELECT TO authenticated USING (bucket_id = 'chat-photos');

DROP POLICY IF EXISTS "Upload Chat Photos" ON storage.objects;
CREATE POLICY "Upload Chat Photos" ON storage.objects 
  FOR INSERT TO authenticated WITH CHECK (bucket_id = 'chat-photos');

-- RESIDENT GALLERY
DROP POLICY IF EXISTS "View Gallery" ON storage.objects;
CREATE POLICY "View Gallery" ON storage.objects 
  FOR SELECT TO authenticated USING (bucket_id = 'resident-gallery');

DROP POLICY IF EXISTS "Staff Upload Gallery" ON storage.objects;
CREATE POLICY "Staff Upload Gallery" ON storage.objects 
  FOR INSERT TO authenticated WITH CHECK (
    bucket_id = 'resident-gallery' AND 
